...
static int map_mark(const race_ui_t *r,
...
static void draw_map(const race_ui_t *r, const race_ui_state *s, const frame *f)
{
    ...
    /* The plate, then the map inset in it. */
    map_panel(px, py, side, RUI_PLATE_GROW, 0,
              0.f, 0.f, 0.f, RUI_PLATE_ALPHA);
    map_panel(px, py, side, 1.f, r->tex.map, 1.f, 1.f, 1.f, 1.f);
    ...
    for (i = 0; i < s->n_cp && i < RUI_MAX_CP; i++)
        map_mark(r, px, py, side, s->cp_x[i], s->cp_z[i], r->tex.cp,
                 i == s->cp_next ? 0 : 1, 0.f, 1.f, 1.f, 1.f, 1.f);

    /* The opponents in map_arrow's second cell, unrotated: the recorded pose's
       heading is available but an opponent's marker is a dot on this art, and
       turning it would claim a precision the 13 px sprite does not have. */
    for (i = 0; i < s->n_others && i < RUI_MAX_OTHERS; i++)
        map_mark(r, px, py, side, s->other_x[i], s->other_z[i],
                 r->tex.arrow, 1, 0.f, 1.f, 1.f, 1.f, 1.f);

    /* The player last, so nothing draws over it, and TURNED.
     *
     * The angle is derived rather than tried: the rig's yaw 0 faces world +Z,
     * +Z is DOWN the painted map (v grows with z -- every track's transform
     * comes out a POSITIVE uniform scale, which is one of the three things
     * gen_hud_data.py asserts), and map_arrow's own triangle points UP. A screen
     * heading of `up' turned clockwise by theta is (sin theta, -cos theta); the
     * car's is (sin yaw, cos yaw) in the same (right, down) axes. Equate them:
     * sin theta = sin yaw and cos theta = -cos yaw, so theta = pi - yaw.
     *
     * NOT yaw + pi, which is the same expression only at yaw 0 and mirrors every
     * other heading -- the class of bug traps.md keeps recording. */
    map_mark(r, px, py, side, s->car_x, s->car_z, r->tex.arrow, 0,
             RUI_PI - s->car_yaw, 1.f, 1.f, 1.f, 1.f);
}
                      ...
                      float frac, const char *value, const char *label)
{
    ...
    if (fs.tex) {
        ...
        sf_text_shadowed(&fs, cx - vw * 0.5f, cy - vh, vs,
                         1.f, 1.f, 1.f, 1.f, value);
        ui_rect(cx - rpx * 0.42f, cy + vh * 0.06f, rpx * 0.84f,
                f->s > 1.f ? f->s : 1.f, 1.f, 1.f, 1.f, 0.75f);
        sf_text_shadowed(&fs, cx - lw * 0.5f, cy + vh * 0.20f, ls,
                  ...
                  int screen_w, int screen_h)
{
    ...
    if (big.tex)
        sf_text_shadowed(&big, axR(&f, HUD_LAPNUM_X), ay(&f, HUD_LAPNUM_Y),
                         f.s, 1.f, 1.f, 1.f, 1.f, buf);
    else
        ui_text(axR(&f, HUD_LAPNUM_X), ay(&f, HUD_LAPNUM_Y), 1.5f * f.s,
                1.f, 1.f, 1.f, 1.f, buf);
       ...
       what the shipped screenshot reads. */
    {
        ...
        /* 2.2369363 mph per m/s. */
        snprintf(buf, sizeof buf, "%i", (int)(s->speed * 2.2369363f + 0.5f));
        draw_dial(r, &f, 1, s->speed / smax, buf, "mph");
    }
